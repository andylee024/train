...
import {
  buildNutritionConfig,
  type NutritionHeadlines,
} from "@/app/nutrition/dashboard";

/**
 * Nutrition dashboard client — same engine as PerformanceViews but single-view
 * (no switcher). Editor toolbar + DashboardRenderer.
 */
export function NutritionDashboard({
  headlines,
  bwSeries,
}: {
  headlines: NutritionHeadlines;
  bwSeries: { date: string; bw: number | null; target: number }[];
}) {
  const defaultConfig = useMemo(
    () => buildNutritionConfig(headlines),
    [headlines]
  );
