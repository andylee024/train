...
function parsePlanMarkdown(content: string): Map<string, PlanItem[]> {
  ...
  for (const line of content.split("\n")) {
    ...
    if (headingMatch) {
      ...
      const dayNumberMatch = heading.match(/^Day\s+(\d+)\b/i);
      if (dayNumberMatch) {
        currentDay = `day-${Number(dayNumberMatch[1])}`;
        if (!days.has(currentDay)) days.set(currentDay, []);
        continue;
      }

      const normalizedHeading = heading.toLowerCase();
      if (WEEKDAY_NAME_TO_DAY_NUMBER.has(normalizedHeading)) {
        currentDay = normalizedHeading;
        if (!days.has(currentDay)) days.set(currentDay, []);
        continue;
      }
...
function applyWeightTargetEdit(item: PlanItem, edit: Extract<PlanEdit, { type: "change_weight_target" }>): boolean {
...
export function planToday(): JsonEnvelope<TodayPlan> {
  ...
  const isoDayNumber = getIsoDayNumber(now);

  const resolved = resolvePlanFile();
  if (!resolved.ok) return resolved;

  const days = parsePlanMarkdown(resolved.data.content);
  const lookupKeys = [dayName, `day-${isoDayNumber}`];
  let exercises: PlanItem[] = [];
  for (const key of lookupKeys) {
    const matched = days.get(key);
  ...
  if (exercises.length === 0) {
    return ok({
      week: resolved.data.week,
      day: dayName,
      file: resolved.data.file,
      exercises: [],
    });
  }
