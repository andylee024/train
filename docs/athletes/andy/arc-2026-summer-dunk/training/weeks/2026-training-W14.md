# Week 2026-training-W14  *(new split — v9)*

## Week Header

- Block: training/blocks/2026-07-block-03-peak-realize-test.md
- Block Display: PEAK + REALIZE + TEST
- Arc Week: 14 of 16
- Block Week: 4 of 6
- Date Range: Sun Aug 23 – Sat Aug 29, 2026
- Week Type: **New Oly-forward split** (v9) — peak crest
- Primary Focus: Peak crest on the new split. Squat wave hits its heavy-crest bracket (4×4 @320 — 6-wk cycle wk4), last hard intensity push before taper. Coach DNT wk13–14 re-homed (once dropped). Last high-box depth jumps Sat. Wed BJJ recovery. Fri + Sat optional.

## Source / Sync

- **Coach program:** `2andy program13-14.pdf` → **DNT wk14**, re-homed below. Snatch complex → Tue, both cleans → Thu, no dedicated jerk/press this week so Fri carries no coach Oly (own upper only).
- **Loads:** coach Oly = **%** with lb estimates off recent tops — send true snatch/clean/jerk 1RMs to firm up.
- **Sync rule:** DNT week now = arc week (aligned 2026-07-28). DNT wk14 = W14.

## The Split (v9)

Same as W15 — Sun/Mon own anchors, Tue/Thu/Fri/Sat coach Oly, Wed BJJ-only.

## 7-Day Overview

1. Sun Aug 23: **Heavy Lower** — back-squat wave (4×4, heavy crest — 6-wk cycle wk4)
2. Mon Aug 24: **Heavy Upper** — bench + pull-up (4×8) | 🥋 BJJ
3. Tue Aug 25: **Oly Lift + Mobility Legs** — Hang Muscle Snatch+OHS + Snatch HP (coach) | 🥋 BJJ
4. Wed Aug 26: **BJJ only** *(rest)* | 🥋 BJJ
5. Thu Aug 27: **Oly Lift + Lower** — Hang Clean + Power C&J (coach) + front-squat wave (4×4) | 🥋 BJJ
6. Fri Aug 28: **Oly Lift + Upper** — no coach jerk/press (own upper) *(optional)*
7. Sat Aug 29: **Oly Lift + Jump/Dunk** — last high-box depth jumps *(optional)*

## Sun Aug 23 (SUNDAY) — Heavy Lower

| # | Lift | Scheme |
|---|------|--------|
| 1 | Back Squat *(wk: 4×4, crest)* | 4×4 @ 320 |
| 2a | Deadlift *(month's variation)* | 4×4, RPE 8 ⟷ 2b Horse Stance 3×10 |
| 3 | Giant set: Nordic · Copenhagen · Hip-Thrust · Tailor Pose | 3×8–10 |
| 4 | Full Side Split Stretch | 3×60–90s |

## Mon Aug 24 (MONDAY) — Heavy Upper | BJJ

| # | Lift | Scheme |
|---|------|--------|
| 1a | Bench Press | 4×8 @ 190 ⟷ 1b Weighted Pull-Up 4×8 @ +20 |
| 2a | Push accessory ⟷ 2b Pull accessory | 3×8–10 |
| 3a | Handstand ⟷ 3b Shoulder health | 3×30–45s / 3×12–15 |

Passive leg stretches during rests. **Evening:** 🥋 BJJ.

## Tue Aug 25 (TUESDAY) — Oly Lift + Mobility Legs | BJJ

| # | Lift | Scheme | Source |
|---|------|--------|--------|
| 1 | Hang Muscle Snatch + Overhead Squat | 80% 5×(2+1) (~85 lb) | coach |
| 2 | Snatch High Pull | 85% 4×2 (~115 lb) | coach |
| 3 | Zercher Side Squat | 3×6–8/side | own |
| 4 | Cossack Squat | 3×6–8/side | own |

**Evening:** 🥋 BJJ.

## Wed Aug 26 (WEDNESDAY) — BJJ only *(rest)* | BJJ

No lifting. **Evening:** 🥋 BJJ.

## Thu Aug 27 (THURSDAY) — Oly Lift + Lower *(Front Squat)* | BJJ

| # | Lift | Scheme | Source |
|---|------|--------|--------|
| 1 | Hang Clean | 85–90% 5×(2-1) (~170 lb) | coach |
| 2 | Power Clean & Jerk | 85–90% 5×(2-1) (~155 lb) | coach |
| 3 | Front Squat *(wave)* | 4×4 @ 275 | own |
| 4 | Good Morning | 5×8 | coach acc |

**Evening:** 🥋 BJJ.

## Fri Aug 28 (FRIDAY) — Oly Lift + Upper *(optional)*

| # | Lift | Scheme | Source |
|---|------|--------|--------|
| 1 | *(no coach jerk/press in wk14 — both C&J re-homed to Thu)* | — | — |
| 2 | Your own upper work / pull-up 5×10 | 3×8–10 | own / coach acc |

**📋 Coach reference menu (wk14):** Pull-up 5×10 · Reverse Plank 4×40s · Abdominal 5×12. **Dropped from fixed slots:** Clean Deadlift 90% 5×2 + Back Squat 90% 3×2/80% 2×4 (your Sun/Thu squat waves cover these).

## Sat Aug 29 (SATURDAY) — Oly Lift + Jump/Dunk *(optional)*

| # | Lift | Scheme |
|---|------|--------|
| 1 | Sprints / Approach / **High-box Depth Jumps (last)** | 3×3 @ 30 in |
| 2 | Dunk Attempts | max intent, filmed |

## Logging Checklist

Wave top sets + coach Oly + bw + side-split + BJJ + pain ratings.

## End-of-Week Review (filled Sun)

1. Wins / 2. Misses / 3. Coach vs own / 4. Adjustment: W15 = TAPER — cut volume ~50%, hold intensity, protect the W16 tests.
