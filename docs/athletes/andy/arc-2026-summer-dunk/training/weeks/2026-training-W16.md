# Week 2026-training-W16  *(new split — v9)*

## Week Header

- Block: training/blocks/2026-07-block-03-peak-realize-test.md
- Block Display: PEAK + REALIZE + TEST
- Arc Week: 16 of 18
- Block Week: 4 of 6
- Date Range: Sun Aug 16 – Sat Aug 22, 2026
- Week Type: **New Oly-forward split** (v9) — peak crest
- Primary Focus: Peak crest on the new split. Squat wave hits its heavy-crest bracket (4×4 @320 — 6-wk cycle wk4), last hard intensity push before taper. Coach DNT wk13–14 re-homed (once dropped). Last high-box depth jumps Sat. Wed BJJ recovery. Fri + Sat optional.

## Source / Sync

- **Coach program:** DNT **wk14** (process the wk13–14 drop into W15–W16). 🏋️ slots = **TBD** until it lands.
- **Sync rule:** DNT runs 2 weeks behind the arc.

## The Split (v9)

Same as W15 — Sun/Mon own anchors, Tue/Thu/Fri/Sat coach Oly, Wed BJJ-only.

## 7-Day Overview

1. Sun Aug 16: **Heavy Lower** — back-squat wave (4×4, heavy crest — 6-wk cycle wk4)
2. Mon Aug 17: **Heavy Upper** — bench + pull-up (4×8) | 🥋 BJJ
3. Tue Aug 18: **Oly Lift + Mobility Legs** — coach TBD | 🥋 BJJ
4. Wed Aug 19: **BJJ only** *(rest)* | 🥋 BJJ
5. Thu Aug 20: **Oly Lift + Lower** — coach TBD + front-squat wave (4×4) | 🥋 BJJ
6. Fri Aug 21: **Oly Lift + Upper** — coach TBD *(optional)*
7. Sat Aug 22: **Oly Lift + Jump/Dunk** — last high-box depth jumps *(optional)*

## Sun Aug 16 (SUNDAY) — Heavy Lower

| # | Lift | Scheme |
|---|------|--------|
| 1 | Back Squat *(wk: 4×4, crest)* | 4×4 @ 320 |
| 2a | Deadlift *(month's variation)* | 4×4, RPE 8 ⟷ 2b Horse Stance 3×10 |
| 3 | Giant set: Nordic · Copenhagen · Hip-Thrust · Tailor Pose | 3×8–10 |
| 4 | Full Side Split Stretch | 3×60–90s |

## Mon Aug 17 (MONDAY) — Heavy Upper | BJJ

| # | Lift | Scheme |
|---|------|--------|
| 1a | Bench Press | 4×8 @ 190 ⟷ 1b Weighted Pull-Up 4×8 @ +20 |
| 2a | Push accessory ⟷ 2b Pull accessory | 3×8–10 |
| 3a | Handstand ⟷ 3b Shoulder health | 3×30–45s / 3×12–15 |

Passive leg stretches during rests. **Evening:** 🥋 BJJ.

## Tue Aug 18 (TUESDAY) — Oly Lift + Mobility Legs | BJJ

| # | Lift | Scheme | Source |
|---|------|--------|--------|
| 1 | 🏋️ Coach Oly — snatch pattern | TBD | coach |
| 2 | Zercher Side Squat | 3×6–8/side | own |
| 3 | Cossack Squat | 3×6–8/side | own |

**Evening:** 🥋 BJJ.

## Wed Aug 19 (WEDNESDAY) — BJJ only *(rest)* | BJJ

No lifting. **Evening:** 🥋 BJJ.

## Thu Aug 20 (THURSDAY) — Oly Lift + Lower *(Front Squat)* | BJJ

| # | Lift | Scheme | Source |
|---|------|--------|--------|
| 1 | 🏋️ Coach Oly — clean/jerk | TBD | coach |
| 2 | 🏋️ Coach Oly — snatch/clean pull | TBD | coach |
| 3 | Front Squat *(wave)* | 4×4 @ 275 | own |
| 4 | Coach accessory | TBD | coach acc |

**Evening:** 🥋 BJJ.

## Fri Aug 21 (FRIDAY) — Oly Lift + Upper *(optional)*

| # | Lift | Scheme | Source |
|---|------|--------|--------|
| 1 | 🏋️ Coach Oly — jerk/press | TBD | coach |
| 2–3 | Coach accessories (upper) | TBD | coach acc |

## Sat Aug 22 (SATURDAY) — Oly Lift + Jump/Dunk *(optional)*

| # | Lift | Scheme |
|---|------|--------|
| 1 | Sprints / Approach / **High-box Depth Jumps (last)** | 3×3 @ 30 in |
| 2 | Dunk Attempts | max intent, filmed |

## Logging Checklist

Wave top sets + coach Oly + bw + side-split + BJJ + pain ratings.

## End-of-Week Review (filled Sun)

1. Wins / 2. Misses / 3. Coach vs own / 4. Adjustment: W17 = TAPER — cut volume ~50%, hold intensity, protect the W18 tests.
