...
Walk the user through a structured process that turns raw source materials about a training methodology into a complete style folder under `docs/content/training-styles/<slug>/`, conforming to the canonical 15-section template.

The output mirrors the worked example at [`docs/content/training-styles/vertical-jump/`](../../../docs/content/training-styles/vertical-jump/), built from [`docs/content/training-styles/_template/`](../../../docs/content/training-styles/_template/).
...
1. [`docs/content/training-styles/_template/guide.md`](../../../docs/content/training-styles/_template/guide.md) — the 15-section template with prompts and skeletons
2. [`docs/content/training-styles/_template/exercise-selection.md`](../../../docs/content/training-styles/_template/exercise-selection.md) — the exercise catalog template
3. [`docs/content/training-styles/_template/README.md`](../../../docs/content/training-styles/_template/README.md) — what files belong in a style folder
4. [`docs/content/training-styles/vertical-jump/guide.md`](../../../docs/content/training-styles/vertical-jump/guide.md) — a fully filled-in example
5. [`docs/content/training-styles/README.md`](../../../docs/content/training-styles/README.md) — the boundary between `concepts/` (shared frameworks) and `styles/<x>/` (style-specific instantiation)
...
If a `docs/content/training-styles/<slug>/` folder already exists, ask explicitly whether to overwrite or add to it. Adding to it = read existing files, integrate new sources, update.
...
**Storage:** As content arrives, prepare it for the final `docs/content/training-styles/<slug>/sources/` directory. One file per source. Use stable, kebab-case filenames derived from the source label or URL slug. Suffix with the format type:
...
Work through the canonical template structure (see [`_template/guide.md`](../../../docs/content/training-styles/_template/guide.md)) section by section. For each section, consult `references/section-extraction.md` for what cues to look for.
...
1. **Create the folder structure.** Mirror `_template/`:
   ```
   docs/content/training-styles/<slug>/
   ├── guide.md
...
4. **Write `exercise-selection.md`.** Filter the master library at [`docs/content/training-styles/exercises.md`](../../../docs/content/training-styles/exercises.md) for exercises this style uses. Use the schema from `_template/exercise-selection.md`. Annotate each exercise with style-specific notes pulled from the sources (typical loading, when to use, when to skip).
...
- **Never delete the user's existing source material.** If `docs/content/training-styles/<slug>/sources/` already has files, integrate alongside them — don't overwrite.
